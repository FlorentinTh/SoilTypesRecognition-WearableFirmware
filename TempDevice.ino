...
void setup() {
	...
	blePeripheral.setLocalName("SoilTypesWearable");
...
void loop() {
	...
	if (floorCharacteristic.written()) {
		...
		CommandClass::Instance()->FloorName(s.substring(0, floorCharacteristic.valueLength()));
	}
		...
		if (currentValue >= BUFFER_SIZE) {
			...
			for (int i = 0; i < BUFFER_SIZE - DATA_NUMBER;i+= DATA_NUMBER) {
				b += sprintf(b, "%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%s,%s,%s\n", String(buffer[i]).c_str(), 
					...
					CommandClass::Instance()->FloorName().c_str(),
